...
void dik(ll anode, int n) {
  priority_queue < pair < ll, pair < ll, ll >>, vector < pair < ll, pair < ll, ll>>>, greater < pair < ll, pair < ll, ll
          >> >> q;
  vector<bool> vis(N, 0);
  for (int i = 1; i <= n; i++)
    cost[i] = 1e18;
...
void task_b() {
  ...
  for (int i = 1; i <= n; i++) {
    for (int j = i; j <= n; j++) {
      //   cout << i << " " << j << " " << sm[j] - sm[i - 1] << "\n";
      mop[{j - i, sm[j] - sm[i - 1]}]++;
    }
  }
  ll Ans = 0;
  for (auto x : mop)Ans += ((x.second * 1ll * (x.second - 1)) / 2);
  cout << Ans << "\n";
}
